...
void Neuron::MutateLayer(float mutationChance, float mutationAmount)
{
	...
	//				if (m_outputWeights.at(i).weight>1 || m_outputWeights.at(i).weight < -1)
	//				{
...
void Net::draw()
{
	...
	//void Net::mutate(float chance, float mag)//, float mag)
	//{
	//	for (unsigned i = 0; i < m_layers.size(); i++)
	//	{
	//		for (unsigned j = 0; j < m_layers.at(i).size(); j++)
	//		{
	//			double __chance = RRRRandom::get().floatInRange(0.01, 1);// / 100;
