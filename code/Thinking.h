...
public:
	...
	void mutateW(float chance, float lr);// , float mag);
...
void Neuron::mutateW(float chance, float lr)//, float mag)
{
	for (int i = 0; i < m_outputWeights.size(); i++)
	{
		//double value = RRRRandom::get().floatInRange(-1,1)/5;
		//if (chance > RRRRandom::get().floatInRange(0, 1))
		//{


			//double mutationAmount = RRRRandom::get().floatInRange(-1, 1);
			//double __chance = RRRRandom::get().floatInRange(0, 1);

			//if (__chance < chance)
			//{
				//std::cout << mutationAmount << std::endl;
				//m_outputWeights.at(i).weight += mutationAmount;
				m_outputWeights.at(i).weight += ((static_cast<double>(rand()) / static_cast<double>(RAND_MAX)) * 2 - 1) * lr;
				//m_outputWeights.at(i).weight += value;

				if (m_outputWeights.at(i).weight>1)
				{
...
void Net::mutate(float chance)//, float mag)
{
	for (unsigned i = 0; i < m_layers.size(); i++)
	{
		for (unsigned j = 0; j < m_layers.at(i).size(); j++)
		{
			double __chance = RRRRandom::get().floatInRange(0, 1)/100;
			double lr = RRRRandom::get().floatInRange(0.1, 1);
			if (__chance < chance)
			{
				m_layers.at(i).at(j).mutateW(chance, lr);//, mag);
			}
		}
	}
}
...
Net::Net(const std::vector<unsigned>& topology)
{
	...
	for (unsigned layerNum = 0; layerNum < numLayers; layerNum++)
	{
		...
		//m_layers.back().back().setOutputVal(RRRRandom::get().floatInRange(-1.0f,1.0f)); // 1.0 originally
		m_layers.back().back().setOutputVal(1.0); // 1.0 originally
	}
}
